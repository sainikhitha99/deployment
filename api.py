...
from fastapi import FastAPI, HTTPException, Header, Depends, Body, Request, Query, Response
...
# Ensure an always-on application log file (app.log) is present for easy debugging
api_log_path = os.path.join(LOG_DIR, "app.log")
if not any(getattr(h, "baseFilename", None) == api_log_path for h in logger.handlers):
    _app_fh = logging.FileHandler(api_log_path)
    _app_fh.setLevel(logging.INFO)
    _app_fh.setFormatter(logging.Formatter("%(asctime)s [%(levelname)s] %(name)s: %(message)s"))
    logger.addHandler(_app_fh)

# Module-level debug log file (enable with DEBUG_LOG_FILES=1/true/yes)
DEBUG_LOG_FILES = os.environ.get("DEBUG_LOG_FILES", "").strip().lower() in ("1", "true", "yes")
if DEBUG_LOG_FILES:
    api_debug_path = os.path.join(LOG_DIR, "api.debug.log")
    if not any(getattr(h, "baseFilename", None) == api_debug_path for h in logger.handlers):
        fh = logging.FileHandler(api_debug_path)
...
# Load .env into os.environ (same behavior as other apps)
_BASE_DIR = os.path.dirname(os.path.abspath(__file__))
...
SECRET_KEY = os.environ.get("SECRET_KEY", "ai-dba-dev-key-CHANGE-ME")
app.add_middleware(SessionMiddleware, secret_key=SECRET_KEY)

# Templates (serve UI pages from templates/)
...
AUTHORITY = os.environ.get("AUTHORITY", f"https://login.microsoftonline.com/{AZURE_AD_TENANT_ID or 'common'}")
...
REDIRECT_PATH_SAFE = REDIRECT_PATH or "/getAToken"

# SSL / TLS (for HTTPS)
SSL_CERT = os.environ.get("SSL_CERT", "")
SSL_KEY = os.environ.get("SSL_KEY", "")
SSL_VERIFY = os.environ.get("SSL_CA_BUNDLE", "").strip().lower() != "false"


# MSAL helpers (adapted for FastAPI/Starlette Request.session)
...
def _get_redirect_uri(request: Request) -> str:
    ...
    return str(request.base_url).rstrip("/") + REDIRECT_PATH_SAFE


def _build_auth_url(request: Request, state=None):
...
def _get_token_from_cache(request: Request):
...
def _msal_config_valid() -> bool:
    return bool(AZURE_AD_CLIENT_ID and AZURE_AD_CLIENT_SECRET)


def _fetch_user_photo(request: Request):
    try:
        ...
        me_resp = requests.get(f"{GRAPH_ENDPOINT}/me", headers=headers, verify=SSL_VERIFY, timeout=10)
        if me_resp.ok:
            me_data = me_resp.json()
            request.session["user_display_name"] = me_data.get("displayName", "")
            request.session["user_email"] = me_data.get("mail") or me_data.get("userPrincipalName", "")

        photo_resp = requests.get(f"{GRAPH_ENDPOINT}/me/photo/$value", headers=headers, verify=SSL_VERIFY, timeout=10)
        if photo_resp.ok:
            b64 = base64.b64encode(photo_resp.content).decode("utf-8")
            request.session["user_photo"] = f"data:image/jpeg;base64,{b64}"
        else:
            request.session["user_photo"] = ""
    except Exception:
        request.session["user_photo"] = ""
...
def login(request: Request):
    ...
    auth_url = _build_auth_url(request, state=request.session["state"]) if _msal_config_valid() else None
    flash_messages = request.session.pop("flash_messages", []) if request.session.get("flash_messages") else []
    return templates.TemplateResponse("login.html", {"request": request, "auth_url": auth_url, "flash_messages": flash_messages})


@app.get(REDIRECT_PATH_SAFE)
def auth_callback(request: Request):
    if request.query_params.get("state") != request.session.get("state"):
        return RedirectResponse(url="/login")

    if "error" in request.query_params:
        fm = request.session.get("flash_messages", [])
        fm.append(("danger", f"Authentication error: {request.query_params.get('error_description', 'Unknown error')}"))
        request.session["flash_messages"] = fm
        return RedirectResponse(url="/login")
    ...
    if request.query_params.get("code"):
        ...
        if "error" in result:
            fm = request.session.get("flash_messages", [])
            fm.append(("danger", f"Token error: {result.get('error_description', 'Unknown')}"))
            request.session["flash_messages"] = fm
            return RedirectResponse(url="/login")

        request.session["user"] = result.get("id_token_claims")
        request.session["access_token"] = result.get("access_token")
        _fetch_user_photo(request)
...
def logout(request: Request):
    ...
    return RedirectResponse(f"{AUTHORITY}/oauth2/v2.0/logout?post_logout_redirect_uri={request.base_url}")


@app.get("/", response_class=HTMLResponse)
def web_index(request: Request):
    user = request.session.get("user_display_name") or (request.session.get("user") or {}).get("preferred_username")
...
def verify_api_key(request: Request) -> str:
    """Require an MSAL-authenticated session.

    This project uses MSAL (Azure AD) for interactive authentication. All API
    endpoints that depend on `verify_api_key` now only accept a server-side
    MSAL session (stored in `request.session['user']`). Static API keys and the
    legacy `/api/login` token flow are disabled.
    """
    user = request.session.get("user")
    if not user:
        logger.warning("Authentication failed: No user session found in request.")
        raise HTTPException(status_code=401, detail="Not authenticated. Please sign in via /login (MSAL).")
    ...
    if not identifier:
        identifier = request.session.get("user_email") or request.session.get("user_display_name") or "msal_user"

    logger.info(f"Authenticated user: {identifier}")
    return f"msal:{identifier}"
...
async def login(req: LoginRequest, request: Request) -> Dict[str, Any]:
    ...
    Use the MSAL-based UI sign-in at `/login` (which performs OAuth via Azure AD).
    This endpoint intentionally rejects legacy username/password authentication.
    """
    raise HTTPException(
        status_code=403,
        detail="Username/password login disabled. Please sign in via /login (MSAL)."
    )


@app.post("/api/logout")
async def logout(request: Request) -> Dict[str, Any]:
    """Legacy API logout disabled.

    Use the MSAL-based logout at `/logout` to clear the MSAL session.
    """
    raise HTTPException(
        status_code=403,
        detail="Legacy API logout disabled. Please sign out via /logout (MSAL)."
    )
...
def connect_database(conn: DatabaseConnectionRequest, api_key: str = Depends(verify_api_key)) -> Dict[str, Any]:
    ...
    try:
        ...
        if conn.use_ssh_tunnel:
            # NOTE: SSH tunnel support is temporarily disabled for debugging and investigation.
            # The original code attempted to open an SSH local port-forward using
            # tunnel_manager.open_tunnel(...). That block is intentionally disabled
            # while we troubleshoot connection issues. If a client requests an SSH
            # tunnel, respond with a clear error so callers know the feature is offline.
            logger.warning(
                "SSH tunnel requested for connection %s but SSH tunnel support is temporarily disabled.",
                connection_id,
            )
            raise HTTPException(
                status_code=400,
                detail=(
                    "SSH tunnel support is temporarily disabled for debugging. "
                    "Please connect directly to the database or re-enable SSH tunnel support."
                ),
            )
...
if __name__ == "__main__":
    ...
    # Enable SSL if cert/key files are provided and exist
    ssl_certfile = None
    ssl_keyfile = None
    if SSL_CERT and SSL_KEY:
        if os.path.exists(SSL_CERT) and os.path.exists(SSL_KEY):
            logger.info("SSL enabled using certificate: %s", SSL_CERT)
